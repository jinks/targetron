...
namespace Targetron
{
    ...
    public class Targetron : MonoBehaviour
    {
        ...
        //Window limits
        private static int minWindowWidth = 255;
        private static int maxWindowWidth = 600;
        private static int minWindowHeight = 76;
        private static int maxWindowHeight = 600;

        //Load image files
        private static string root = KSPUtil.ApplicationRootPath.Replace("\\", "/");
        private WWW img1 = new WWW("file://" + root + "GameData/Targetron/Icons/target.png");
        private WWW img2 = new WWW("file://" + root + "GameData/Targetron/Icons/rocket.png");
        private WWW img3 = new WWW("file://" + root + "GameData/Targetron/Icons/nw-resize.png");
        private WWW img4 = new WWW("file://" + root + "GameData/Targetron/Icons/sw-resize.png");

        private WWW img5 = new WWW("file://" + root + "GameData/Targetron/Icons/name_asc.png");
        private WWW img6 = new WWW("file://" + root + "GameData/Targetron/Icons/name_desc.png");
        private WWW img7 = new WWW("file://" + root + "GameData/Targetron/Icons/dist_asc.png");
        private WWW img8 = new WWW("file://" + root + "GameData/Targetron/Icons/dist_desc.png");
        ...
        private static Texture2D contextBGA = new Texture2D(1, 1);
        private static Texture2D contextBGH = new Texture2D(1, 1);
        private static GUIStyle contextStyle = null;
        private static GUIStyle contextStyle2 = null;
        private static GUIStyle buttonStyle = null;
        private static GUIStyle buttonStyle2 = null;
        private static GUIStyle buttonStyle3 = null;
        private static GUIStyle centeredStyle = null;
        private static GUIStyle leftStyle = null;
        private static GUIStyle rightStyle = null;
        private static GUIStyle tooltipStyle = null;
        private static GUIStyle expandStyle = null;
        private static GUIStyle rowStyle = null;
        ...
        private static Color disabledBGColor = Color.gray;

        private static String tooltip = "";
        private static Rect tooltipPos = new Rect(0, 0, 0, 0);  //Window position and size
        private static Target contextActive = null;    //Vessel that was right clicked
        private static Target lcontextActive = null;    //Last vessel that was right clicked
        private static Rect contextPos = new Rect(0, 0, windowWidth, windowHeight);  //Window position and size
        private static ModuleDockingNode activeDockingNode = null; //Vessel that is being hovered over in context menu
        private static ModuleDockingNode lastActiveDockingNode = null; //Vessel that is being hovered over in context menu

        private static int windowWidth = 350;    //Window width
        private static int windowHeight = 194;   //Window height
        ...
        private static int top = 0;    //Measures height of target list
        ...
        private static int sortMode = 0;    //0 = Distance Ascending, 1 = Distance Descending, 2 = Name Ascending, 3 = Name Descending
        private static bool inFlight = false;    //Toggle state for window
        private static String searchStr = "";   //Current search string
        ...
        private static bool handleClicked = false;         //Bottom right resize handle active?
        private static bool handleClicked2 = false;        //Bottom left resize handle active?
        private static bool resetCursor = true;
        private static Vector3 clickedPosition = new Vector3();    //Position where resize handle was originally clicked
        ...
        public void Start()
        {
            config = KSP.IO.PluginConfiguration.CreateForType<Targetron>();
            config.load();
            //Load position and collapse state from config file
            pos = config.GetValue<Rect>("pos", new Rect(Screen.width - windowWidth - 10, Screen.height / 3.0f - windowHeight / 2.0f, windowWidth, windowHeight));
            expand = config.GetValue<bool>("expand", true);
            toggleOn = config.GetValue<bool>("toggleOn", true);
            sortMode = config.GetValue<int>("sortMode", 0);

            for (int i = 0; i < filters.Count; i++)
                filters[i].Enabled = config.GetValue<bool>("filter" + i,true);
        ...
        public void Update()
        {
            if (FlightGlobals.fetch != null && FlightGlobals.fetch.vessels != null && FlightGlobals.fetch.activeVessel != null)  //Check if in flight
            {
                ...
                if (expand && toggleOn && System.DateTime.Now.Ticks > lastChecked + 10000000)
                {
                    lastChecked = System.DateTime.Now.Ticks;    //Save the list check time
                    ...
                    else if (sortMode == 2)
                    {
                        ...
                        targets.Sort((x, y) => string.Compare(x.vessel.GetName(), y.vessel.GetName()));
                    }
                    else if (sortMode == 3)
                    {
                        targets.Sort(sortByDistanceA);
                        targets.Sort((y, x) => string.Compare(x.vessel.GetName(), y.vessel.GetName()));
                    }
                    ...
                    foreach(Target t in targets)
                    {
                        ...
                        if (t.vessel.loaded)    //Vessel has parts loaded
                        {
                            ...
                            for (int i = 0; i < 2; i++)
                            {
                                foreach (Part p in t.vessel.parts)  //Iterate through all parts
                                {
                                    ...
                                    {
                                        if (m.ClassName.Equals("ModuleDockingNode"))   //Check if module is docking node
                                        {
        ...
        private void OnDraw()
        {
            if (toggleOn && inFlight)  //Check if in flight
            {
                ...
                //Display the window
                pos = GUI.Window(WINDOWID_GUI, pos, drawTargeter, "");
                ...
                if (contextActive != null)
                {
                    if (!contextActive.Equals(lcontextActive))
                    {
                        ...
                        float top = Screen.height - Input.mousePosition.y + 10;
                        if (left + textSize.x + 5 > Screen.width)
                            left = Screen.width - textSize.x - 5;
                        if (left < 0)
                            left = 0;
                        if (top + (numDocks+3) * Mathf.RoundToInt(contextStyle.fixedHeight) > Screen.height)
                            top = Screen.height - Mathf.RoundToInt(contextStyle.fixedHeight) - numDocks * Mathf.RoundToInt(contextStyle.fixedHeight);
                        if (top < 0)
                            top = 0;
                        contextPos = new Rect(left, top, textSize.x + 5,(numDocks+3) * Mathf.RoundToInt(contextStyle.fixedHeight));
                        lcontextActive = contextActive;
                    }
                    contextPos = GUI.Window(WINDOWID_CONTEXT, contextPos, drawContext, "");
                ...
                if (contextActive == null && tooltip != "")
                {
                    Vector2 textSize = tooltipStyle.CalcSize(new GUIContent(tooltip));
                    float left = Input.mousePosition.x + 5;
                    float top = Screen.height - Input.mousePosition.y - 35;
                    if (left + textSize.x + 10 > Screen.width)
                        left = Screen.width - textSize.x - 10;
                    if (top < 0)
                        top = 0;
                    tooltipPos = new Rect(left, top, textSize.x + 10, textSize.y - 3);
                    tooltipPos = GUI.Window(WINDOWID_TOOLTIP, tooltipPos, drawTooltip, "");
        ...
        {
            int top = 0;
            if (GUI.Button(new Rect(0, top, contextPos.width, 20), "Target Vessel", contextStyle))
            {
                FlightGlobals.fetch.SetVesselTarget(contextActive.vessel);
                contextActive = null;
            }
            top += Mathf.RoundToInt(contextStyle.fixedHeight);
            if (GUI.Button(new Rect(0, top, contextPos.width, 20), "Control Vessel", contextStyle))
            {
                saveConfig();
                //GamePersistence.SaveGame("persistent", HighLogic.SaveFolder, SaveMode.OVERWRITE);
                contextActive = null;
                FlightGlobals.SetActiveVessel(contextActive.vessel);
            }
            top += Mathf.RoundToInt(contextStyle.fixedHeight);
            String renameStr = "Rename Vessel";
            String renameHint = "";
            if (!contextActive.vessel.loaded)
            {
            ...
            if (GUI.Button(new Rect(0, top, contextPos.width, 20), new GUIContent(renameStr,renameHint), contextStyle))
            {
                contextActive.vessel.RenameVessel();
                contextActive = null;
            }
            GUI.enabled = true;
            ...
            if (contextActive != null)
            {
                ...
                foreach (ModuleDockingNode m in contextActive.availableDocks)
                {
                    ...
                    if (new Rect(0, top, contextPos.width, 20).Contains(Event.current.mousePosition))
                    {
                    ...
                    if (GUI.Button(new Rect(0, top, contextPos.width, 20), "Target " + m.part.partInfo.title, temp))
                    {
        ...
        private static int sortByDistance(Target a, Target b)
        {
            ...
            if (a.vessel == null && b.vessel == null)
                return 0;
            else if (a.vessel == null && b.vessel != null)
                return 1;
            else if (a.vessel != null && b.vessel == null)
                return -1;
            if (a.vessel.orbit != null && FlightGlobals.fetch.activeVessel.orbit.referenceBody == a.vessel.orbit.referenceBody)
            {
                if (b.vessel.orbit == null || a.vessel.orbit.referenceBody != b.vessel.orbit.referenceBody)
                    return -1;
            }
            if (b.vessel.orbit != null && FlightGlobals.fetch.activeVessel.orbit.referenceBody == b.vessel.orbit.referenceBody)
            {
                if (a.vessel.orbit == null || a.vessel.orbit.referenceBody != b.vessel.orbit.referenceBody)
                    return 1;
            }
        ...
        private void drawTargeter(int windowID)
        {
            ...
            expand = GUI.Toggle(new Rect(pos.width - 20, 4, 20, 20), expand, new GUIContent("", tooltipText), expandStyle);
            ...
            if (expand) //If window is expanded
            {
                ...
                foreach (Target target in targets)  //Iterate through each target vessel
                {
                    ...
                    if (showVessel)
                    {
                        ...
                        try
                        {
                            ...
                            GUILayout.Label(name, leftStyle, GUILayout.Width(diff), GUILayout.Height(24));
               ...
               /* for (int i = 0; i < 50; i++)
                ...
                for (i = 0; i < filters.Count; i++)
                {
                    ...
                    if (GUI.Button(new Rect(pos.width - 28 - 23 * i, pos.height - 25, 24, 24), new GUIContent(filters[i].Texture, filters[i].getName()), buttonStyle2))
                    {
                        filters[i].toggle();
                        if (Event.current.button == 1)
                        {
        ...
        public void initStyles()
        {
            ...
            buttonStyle.margin = new RectOffset(2, 0, 0, 0);
            buttonStyle.fixedWidth = 18;
            buttonStyle.fixedHeight = 18;

            buttonStyle2 = new GUIStyle(GUI.skin.GetStyle("Button"));
            buttonStyle2.padding = new RectOffset(0, 0, 0, 0);
            buttonStyle2.margin = new RectOffset(1, 0, 0, 0);
            buttonStyle2.fixedWidth = 22;
            buttonStyle2.fixedHeight = 22;

            buttonStyle3 = new GUIStyle(GUI.skin.GetStyle("Button"));
            buttonStyle3.padding = new RectOffset(2, 2, 2, 2);
            buttonStyle3.margin = new RectOffset(2, 0, 0, 0);
            buttonStyle3.fixedWidth = 20;
            ...
            contextStyle.fontStyle = FontStyle.Normal;
            contextStyle.normal.background = contextBGN;
            contextStyle.normal.textColor = new Color(1.0f, 0.858f, 0.0f); //Orange
            contextStyle.hover.background = contextBGH;
            contextStyle.hover.textColor = new Color(1.0f, 1.0f, 0.0f); //Yellow
            contextStyle.active.background = contextBGA;
            contextStyle.active.textColor = new Color(0.858f, 0.736f, 0.0f); //Dark Orange
            contextStyle.alignment = TextAnchor.MiddleLeft;
            contextStyle.padding = new RectOffset(5, 5, 3, 0);
            contextStyle.margin = new RectOffset(0, 0, 0, 0);
            contextStyle.wordWrap = false;
            contextStyle.fixedHeight = 18;
            contextStyle.clipping = TextClipping.Clip;

            contextStyle2 = new GUIStyle(contextStyle);
            contextStyle2.normal.textColor = new Color(0.858f, 0.0f, 0.0f); //Red
            contextStyle2.hover.textColor = new Color(0.9f, 0.2f, 0.2f); //Light red
            contextStyle2.active.textColor = new Color(0.758f, 0.0f, 0.0f); //Dark red
            ...
            expandStyle.fixedWidth = 12;
            expandStyle.fixedHeight = 12;

            centeredStyle = new GUIStyle();
            centeredStyle.padding = new RectOffset(0, 0, 0, 0);
            centeredStyle.margin = new RectOffset(0, 0, 0, 0);

            leftStyle = new GUIStyle(GUI.skin.GetStyle("Label"));
            leftStyle.margin = new RectOffset(0, 2, 0, 0);
            ...
            rightStyle.margin = new RectOffset(2, 2, 0, 0);
            rightStyle.alignment = TextAnchor.UpperRight;
            rightStyle.clipping = TextClipping.Overflow;
    ...
    class Target
    {
        ...
        public void update()
        {
            ...
            lastVName = this.vessel.GetName();
            lastVType = this.vessel.vesselType;
            try
            {
                this.lastDistance = Vector3.Distance(FlightGlobals.fetch.activeVessel.transform.position, vessel.transform.position);
            }
            catch (Exception e)
            {
